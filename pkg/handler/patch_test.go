...
func TestPatch(t *testing.T) {
	...
	SubTest(t, "UploadNotFoundFail", func(t *testing.T, store *MockFullDataStore, composer *StoreComposer) {
		store.EXPECT().GetUpload(WrapsContext(context.Background()), "no").Return(nil, ErrNotFound)
	...
	SubTest(t, "NotifyUploadProgress", func(t *testing.T, store *MockFullDataStore, composer *StoreComposer) {
		...
		gomock.InOrder(
			...
			upload.EXPECT().WriteChunk(WrapsContext(context.Background()), int64(10), NewReaderMatcher("first second third")).Return(int64(18), nil),
		)
	...
	SubTest(t, "StopUpload", func(t *testing.T, store *MockFullDataStore, composer *StoreComposer) {
		...
		gomock.InOrder(
			...
			upload.EXPECT().WriteChunk(WrapsContext(context.Background()), int64(0), NewReaderMatcher("first ")).Return(int64(6), nil),
